...
local is_sap = function(opts, driver, device)
  for _, fingerprint in ipairs(SAP_FINGERPRINTS) do
...
local function fan_speed_handler(driver, device, command)
  if command.args.speed == 4 then command.args.speed = 5 end
...
local function zb_fan_control_handler(driver, device, value, zb_rx)
  if value.value == 5 then value.value = 4 end
